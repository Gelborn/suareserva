...
const formatPrice = (cents?: number | null) => (cents == null ? 'Sob consulta' : BRL.format(cents / 100));
...
const ProgressBar: React.FC<{ step: Step }> = ({ step }) => (
  <div className="flex items-center gap-2">
    {[1, 2, 3, 4].map((current) => {
      const status = current < step ? 'done' : current === step ? 'current' : 'upcoming';
      return (
  ...
  <aside className={clsx('rounded-3xl border border-gray-200 bg-white/80 p-6 shadow-sm backdrop-blur', className)}>
    ...
    <div className="mt-6 space-y-4 text-sm text-gray-700">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="text-right font-semibold text-gray-900">{service?.name ?? 'Selecione um serviço'}</span>
      </div>
      <div className="flex items-start justify-between gap-4">
        <span className="text-gray-500">Profissional</span>
        <span className="text-right text-gray-900">{provider?.full_name ?? 'Selecione um profissional'}</span>
      </div>
      <div className="flex items-start justify-between gap-4">
...
const SuccessView: React.FC<{
  ...
  onNew: () => void;
  accent: string;
}> = ({ service, provider, slotLabel, slotDate, onNew, accent }) => (
  <div className="space-y-6 rounded-3xl border border-emerald-200 bg-emerald-50/80 p-8 text-emerald-900">
    <div className="flex items-center gap-3">
      <CheckCircle2 className="h-6 w-6" />
      <h3 className="text-xl font-semibold">Pedido enviado!</h3>
    </div>
    <p className="text-sm">Confirmaremos sua reserva e enviaremos avisos por WhatsApp para você acompanhar.</p>
    <div className="rounded-2xl border border-emerald-200 bg-white/70 p-4 text-sm text-gray-700">
      <p><span className="text-gray-500">Serviço:</span> <strong>{service?.name ?? '—'}</strong></p>
      <p><span className="text-gray-500">Profissional:</span> <strong>{provider?.full_name ?? '—'}</strong></p>
      <p><span className="text-gray-500">Horário:</span> <strong>{slotLabel ?? '—'}</strong>{slotDate ? <span className="text-gray-500"> • {slotDate}</span> : null}</p>
    </div>
    <button
      type="button"
      onClick={onNew}
...
const PublicStorePage: React.FC = () => {
  ...
  const handleCreateBooking = async () => {
    ...
    try {
      ...
      toast.success('Confirmaremos sua reserva e enviaremos avisos por WhatsApp.');
    } catch (err: any) {
      console.error(err);
  ...
  if (loading) return <LoadingScreen message="Buscando informações da loja…" />;
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-950 to-black text-white">
        <div className="mx-auto flex min-h-screen max-w-3xl flex-col items-center justify-center px-6 text-center">
          ...
          <p className="mt-3 text-sm text-slate-300">Não conseguimos localizar uma loja com o endereço solicitado. Verifique o link ou fale com o estabelecimento.</p>
        </div>
      </div>
    );
  }
  ...
  const desktopConfirmView = step === 4 && showSummary;
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-gradient-to-br from-slate-50 via-white to-slate-100 text-slate-900">
      ...
      <main className="relative z-10 mx-auto max-w-5xl px-6 pb-24">
        {error && <div className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">{error}</div>}

        <div className={clsx('mt-12', desktopConfirmView ? 'max-w-xl mx-auto' : 'grid gap-10 lg:grid-cols-[minmax(0,1fr)_320px]')}>
          {!desktopConfirmView ? (
            <>
                  ...
                  {step === 2 && (
                    <div className="space-y-6">
                      {selectedService?.providers.length === 0 ? (
                        <EmptyState title="Sem profissionais vinculados" message="Esse serviço ainda não possui profissionais disponíveis. Volte e escolha outro serviço ou tente mais tarde." />
                      ) : (
                        <div className="space-y-4">
                          {selectedService?.providers.map((provider) => (
                            <ProviderCard key={provider.id} provider={provider} active={provider.id === selectedProviderId} onSelect={() => setSelectedProviderId(provider.id)} accent={accent} />
                          ))}
                        </div>
                      )}
                              ...
                              {availability.days.map((day) => {
                                ...
                                return (
                                  <button
                                    ...
                                    disabled={!hasSlots}
                                    className={clsx(
                                      'shrink-0 w-28 rounded-2xl border px-3 py-2 text-left transition',
                                      isActive ? 'border-transparent bg-gray-900 text-white shadow-lg' : 'border-gray-200 bg-white text-gray-800 hover:border-gray-300',
                                      !hasSlots && 'cursor-not-allowed opacity-60'
                                    )}
                                  >
                                    <div className={clsx('flex items-center gap-2 text-[11px] uppercase tracking-wide', labelMuted)}>
                                      <Calendar className="h-3.5 w-3.5" />
                                      {day.weekday}
                                    </div>
                                    <div className="mt-1 text-xl font-semibold">{day.dayNumber}</div>
                                    <p className={clsx('mt-0.5 text-[11px]', labelMuted)}>{day.fullLabel}</p>
                                    {!hasSlots && <p className={clsx('mt-2 text-[11px]', labelMuted)}>Sem horários</p>}
                                  </button>
                                );
                              })}
                            </div>
                              ...
                              {availability.slotsByDay[selectedDayKey].map((slot) => (
                                <button
                                  ...
                                  className={clsx(
                                    ...
                                    slot.isoStart === selectedSlotIso ? 'bg-gray-900 text-white shadow' : 'bg-white text-gray-900 border-gray-200 hover:border-gray-300'
                                  )}
                                >
                                  {slot.label}
                                </button>
                            ...
                            <div>
                              ...
                              <input
                                ...
                                className={clsx(
                                  ...
                                  phoneValid ? 'border-gray-200 focus:ring-gray-900/10' : 'border-amber-300 focus:border-amber-400 focus:ring-amber-200'
                                )}
                              />
                              <p className="mt-1 text-xs text-gray-500">Confirmaremos sua reserva e enviaremos avisos por WhatsApp/SMS.</p>
                            </div>
